...
const CITY_API = 'http://api.travelpayouts.com/data/ru/cities.json',
    ...
    API_KEY = '5bd3032469048d67f03c5b21dc158c5b',
...
const renderCheapDay = (cheapTicket) => {
    cheapestTicket.style.display = '';
    cheapestTicket.innerHTML = '<h2>Самый дешевый билет на выбранную дату</h2>';

    const ticket = createCards(cheapTicket[0]);
    cheapestTicket.append(ticket);
};

const renderCheapYear = (cheapTickets) => {
    otherCheapTickets.style.display = '';
    otherCheapTickets.innerHTML = '<h2>Самые дешевые билеты на другие даты</h2>';

    cheapTickets.sort((a, b) => a.value - b.value);

    for (let i = 0; i < cheapTickets.length && i < MAX_COUNT; i++) {
        const ticket = createCards(cheapTickets[i]);
